...
function recognize(s, context) {
  const filtered = s.replace(/\(|\)|lat(itude)?|lon(g(itude)?)?|lng|gps|geo|coordinates?|maps?/, '');

  //console not supported yet
  //console.log('Filtered query = "' + filtered + '"');

  const words = filtered.split(/\s+|\s*,\s*/);

  if (words.length < 2) {
    return null;
  }

  const latitude = parseFloat(words[0]);
  const longitude = parseFloat(words[1]);
