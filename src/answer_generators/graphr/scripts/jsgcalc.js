...
function JSgCalc (element){
	...
	this.resizeGraph = function(width, height) {
		...
		this.maxgridlines.x = 0.04 * width;
		this.maxgridlines.y = 0.04 * height;
		this.draw();
	}

	this.resetZoom = function() {
		this.currCoord = {x1 : -5 * (this.width / this.height), y1 : -5, x2 : 5 * (this.width / this.height), y2 : 5.1};
		this.startCoord = this.copyCoord(this.currCoord);
		this.draw();
	};
	...
	this.initCanvas = function() {
		if (this.graph.getContext){
            ...
            var ratio = (this.width || 1) / (this.height || 1)
			this.currCoord = {x1 : -5 * ratio, y1 : -5, x2 : 5 * ratio, y2 : 5};
			this.startCoord = this.copyCoord(this.currCoord);
			jsgui.evaluate();
			...
			$('#graph_wrapper').text("Sorry, your browser is not supported.");
		}
	};

	this.resize = function() {
        var graphWrapperWidth = Math.max($(window).width() ||
          $("#graph_wrapper").width() || DEFAULT_WIDTH, 50);
        var graphWrapperHeight = Math.max(($(window).height() ||
          $("#graph_wrapper").height() || DEFAULT_HEIGHT) - NON_GRAPH_HEIGHT, 50);
        this.resizeGraph(graphWrapperWidth, graphWrapperHeight);
	}
};
